...
const Dashboard: React.FC = () => {
  ...
  const [isDark, setIsDark] = useState(() => {
    try { return localStorage.getItem('vocab_dark_mode') === 'true'; } catch { return false; }
  });

  // Sync dark mode with body class
  useEffect(() => {
    document.body.classList.toggle('dark', isDark);
    localStorage.setItem('vocab_dark_mode', String(isDark));
  }, [isDark]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Faqat SHU BATCH ichida dublikatlarni tekshiramiz (boshqa bo'limlardagi so'zlar to'smaydi)
      const existingWordsInBatch = new Set(
        flashcards
          .filter(c => c.batchId === batchId && c.language === targetLanguage)
          .map(c => c.word.toLowerCase())
      );
      const uniqueNewCards = allNewCards.filter((c) => !existingWordsInBatch.has(c.word.toLowerCase()));

      const addedCount = uniqueNewCards.length;
      const skippedCount = allNewCards.length - uniqueNewCards.length;

      if (skippedCount > 0 && addedCount > 0) {
        setNotification({ message: `${addedCount} ta so'z qo'shildi, ${skippedCount} ta o'tkazib yuborildi (bugungi ro'yxatda bor edi).`, type: 'success' });
      } else if (skippedCount > 0 && addedCount === 0) {
        setNotification({ message: `Barcha so'zlar bugungi ro'yxatda allaqachon bor!`, type: 'error' });
      } else {
        setNotification({ message: `${addedCount} ta so'z muvaffaqiyatli qo'shildi!`, type: 'success' });
      }
  ...
  return (
    <div className={`min-h-screen font-sans relative transition-colors duration-300 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-gray-50 text-slate-900'}`}>
      {/* PWA INSTALL BANNER */}
      ...
      {(isLoading || flashcardsLoading) && (
        <div className={`fixed inset-0 z-[70] backdrop-blur-md flex flex-col items-center justify-center animate-in fade-in duration-500 ${isDark ? 'bg-slate-900/70' : 'bg-white/50'}`}>
      ...
      {batchToDelete && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className={`rounded-2xl shadow-2xl max-w-sm w-full p-6 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
            <div className="flex flex-col items-center text-center">
              <h3 className={`text-lg font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>O'chirishni tasdiqlang</h3>
      ...
      <div className={`transition-all duration-300 ${isBannerVisible ? 'pt-16' : ''}`}>
        {/* Header */}
        <header className={`text-white py-6 px-4 transition-colors duration-300 ${isDark ? 'bg-slate-950 border-b border-slate-800' : 'bg-slate-900'}`}>
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            {/* LANGUAGE SWITCHER */}
            <div className={`p-1 rounded-lg flex gap-1 ${isDark ? 'bg-slate-900' : 'bg-slate-800'}`}>
              <button
            ...
            <div className="flex gap-2 items-center">
              ...
              <button
                ...
                className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all ${isDark ? 'bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
        ...
        <main className="max-w-4xl mx-auto w-full px-4 py-8 pb-32">
          {/* Input Section */}
          <section className={`p-6 rounded-xl border shadow-sm mb-10 relative overflow-hidden transition-colors duration-300 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            {/* Background Flag Hint */}
            <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
              <span className={`text-9xl font-black ${isDark ? 'text-slate-700' : 'text-slate-900'}`}>{targetLanguage === 'en' ? 'EN' : targetLanguage === 'es' ? 'ES' : 'ZH'}</span>
            </div>

            <div className="flex justify-between items-center mb-3">
              <label className={`block text-[10px] font-black uppercase tracking-[0.15em] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                Add Words ({targetLanguage === 'en' ? 'English' : targetLanguage === 'es' ? 'Spanish' : 'Chinese/Hanzi'})
              </label>
              <button
                onClick={() => setShowOcrModal(true)}
                className={`text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1 ${isDark ? 'text-indigo-400 hover:text-indigo-300 bg-indigo-900/30 hover:bg-indigo-900/50' : 'text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100'}`}
              >
            ...
            <div className="flex gap-4 items-start z-10 relative">
              <textarea
                className={`flex-1 p-4 border rounded-lg focus:ring-1 outline-none h-32 text-sm font-medium resize-none transition-colors duration-300 ${isDark ? 'bg-slate-700 border-slate-600 text-white placeholder:text-slate-500 focus:ring-indigo-500' : 'bg-white border-slate-200 text-slate-900 placeholder:text-slate-300 focus:ring-slate-900'}`}
              ...
              <button
                ...
                className={`h-32 px-6 rounded-lg font-black text-xs tracking-widest uppercase transition-all flex flex-col items-center justify-center gap-2 ${isLoading ? (isDark ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-slate-100 text-slate-400 cursor-not-allowed') : 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-md shadow-indigo-200'}`}
              >
                {isLoading ? (
                  <span>...</span>
                ) : (
          ...
          <footer className="mt-20 mb-8 text-center flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
            <a href="https://t.me/MR_Odilxon" target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center gap-2 hover:scale-105 transition-transform">
              <div className={`p-1 rounded-full border shadow-sm group-hover:shadow-md transition-shadow ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
