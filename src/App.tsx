...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-slate-900 relative">
      ...
      {/* NOTIFICATION TOAST */}
      {notification && (
        <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] px-6 py-3 rounded-full shadow-2xl animate-in slide-in-from-top fade-in duration-300 ${notification.type === 'success' ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'
          }`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {filteredBatches.map(([id, cards]) => {
                  ...
                  return (
                    <div
                      key={id}
                      className={`relative bg-white border rounded-xl p-5 transition-all hover:border-indigo-200 hover:shadow-md animate-in fade-in zoom-in-95 duration-300 ${selectedBatchIds.has(id) ? 'border-indigo-500 ring-1 ring-indigo-500 bg-indigo-50/10' : 'border-slate-200'
                        }`}
                    >
