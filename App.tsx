...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    // 1. Split by newlines first to respect list format
    // 2. Clean up "1. Word - Translation" formats
    // 3. Split by commas if it's a simple list
    const rawLines = rawInput.split('\n');
    let extractedWords: string[] = [];

    rawLines.forEach(line => {
      // Remove leading numbers (e.g., "1.", "10)")
      let cleanLine = line.replace(/^\d+[\.\)]\s*/, '').trim();
      
      if (!cleanLine) return;

      // Handle "Word - Translation" or "Word – Translation"
      // Split by hyphen/dash and take the first part
      const parts = cleanLine.split(/[\-–—]/);
      let potentialWord = parts[0].trim();

      // If the line didn't have a hyphen, it might be "Apple, Banana, Pear"
      // So we split by comma
      const commaSeparated = potentialWord.split(',');
      
      commaSeparated.forEach(w => {
        const finalWord = w.trim();
        // Check if it's not empty and contains at least some letters (not just symbols)
        if (finalWord.length > 0 && /[a-zA-Z]/.test(finalWord)) {
           extractedWords.push(finalWord);
        }
      });
    });

    // Remove duplicates from current input and existing cards
    const uniqueNewWords = extractedWords.filter(
      w => !flashcards.some(existing => existing.word.toLowerCase() === w.toLowerCase())
    );

    // Remove duplicates within the input itself
    const finalWordsToProcess = Array.from(new Set(uniqueNewWords));

    if (finalWordsToProcess.length === 0) {
      setNotification({ message: "Yangi so'zlar topilmadi yoki barchasi mavjud.", type: 'error' });
      return;
    }

    setIsLoading(true);
    try {
      // Process in chunks (internal batching for API stability)
      const chunkSize = 15; 
      for (let i = 0; i < finalWordsToProcess.length; i += chunkSize) {
        ...
        const timestamp = new Date();
        const batchId = `${timestamp.getFullYear()}-${String(timestamp.getMonth() + 1).padStart(2, '0')}-${String(timestamp.getDate()).padStart(2, '0')} Batch ${Math.floor(i/chunkSize) + 1}`;
      ...
      setInput('');
      setNotification({ message: "So'zlaringiz tayyor!", type: 'success' });
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-slate-900 relative">
      ...
      {isLoading && (
        <div className="fixed inset-0 z-[70] bg-white/50 backdrop-blur-md flex flex-col items-center justify-center animate-in fade-in duration-500">
          ...
          <p className="text-slate-500 text-sm mt-2">So'zlar tahlil qilinmoqda</p>
        </div>
      )}
